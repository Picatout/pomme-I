...
;---------------------------------
	...
	NBR_COL=WCNT+1 ; display columns 
	WIDTH_DIV=NBR_COL+1 ; modulo divisor for column width 
	YSAVE=WIDTH_DIV+1
	XSAVE=YSAVE+2 
	VSIZE=XSAVE+1  	
cmd_words:
	_vars VSIZE
	ld a,#4 ; default to 4 columns 
	ld (COL_CNT,sp),a 
	ld (NBR_COL,sp),a 
	clr (WCNT,sp)
	ld a,#11 ; default width 10 characters + 1 
	ld (WIDTH_DIV,sp),a ; modulo divisor 
	_clrz acc16  
	ldw (YSAVE,sp),y 
	ld a,(y)
	cp a,#LITC_IDX 
...
0$: 
	ldw (YSAVE,sp),y 
	ld a,#3 ; 3 columns when showing token bytecode
	ld (COL_CNT,sp),a  	
	ld (NBR_COL,sp),a 
	ld a,#10 ; column width 13 characters - 4 + 1
	ld (WIDTH_DIV,sp),a ; modulo divisor 
